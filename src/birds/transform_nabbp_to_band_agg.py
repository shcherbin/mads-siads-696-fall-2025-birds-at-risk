...
def aggregate_band_data(df: pl.DataFrame) -> pl.DataFrame:
    ...
    banding_df = df.filter(pl.col('event_type')=='B')[[
        'record_id','band','event_date', 'event_day','event_month',
        'event_year', 'species_id', 'iso_country', 
    ...
    banding_df = banding_df.rename({
        ...
        'event_day':'banding_day',
        'event_month':'banding_month',
        'event_year':'banding_year',
...
def add_num_event_days_span(df):
    import polars as pl
    

    # add new column num_event_days_span = event_days_span * num_events
    df = df.with_columns(
        (pl.col("event_days_span") * pl.col("num_events")).alias("num_event_days_span")
    )
